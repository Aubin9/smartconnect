          ...
          <Card className="p-4">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <CardDescription>
                  Color-coded cell sectors: green good, yellow warning, red
                  congested, purple predicted congestion.
                </CardDescription>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <Button variant="outline" size="sm">
                  <RadioTower className="h-4 w-4" /> Operator
                </Button>
                <Button variant="outline" size="sm">
                  Historical playback
                </Button>
          ...
          <Card>
            <CardHeader>
              <div>
                ...
                <CardDescription>
                  Next 30-minute LSTM forecast window
                </CardDescription>
              </div>
            </CardHeader>
            <div className="space-y-3">
              {predictions.map((prediction) => (
                <div
                  key={prediction.id}
                  className="rounded-2xl border border-slate-100 bg-gradient-to-r from-amber-50 to-red-50 p-3 dark:border-slate-800 dark:from-slate-900 dark:to-slate-800"
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-black text-slate-900 dark:text-white">
                        {prediction.cellSector.cellId}
                      </p>
                      <p className="text-xs text-slate-500">
                        {prediction.cellSector.location} •{" "}
                        {prediction.windowMinutes} min
                      </p>
                    </div>
                    <Badge
                      tone={
                        prediction.probability >= 0.85 ? "danger" : "warning"
                      }
                    >
      ...
      <Card>
        <CardHeader>
          <div>
            ...
            <CardDescription>
              Last 50 alerts, credits, loans and insurance actions.
            </CardDescription>
          </div>
          <Button variant="outline" size="sm">
            Export PDF <ArrowRight className="h-4 w-4" />
          </Button>
        </CardHeader>
        <div className="max-h-96 space-y-3 overflow-y-auto pr-2 scrollbar-thin">
          {actions.map((action) => (
            <div
              key={action.id}
              className="flex gap-3 rounded-2xl bg-slate-50 p-3 dark:bg-slate-800/60"
            >
              <div className="mt-1">
                {action.kind.includes("credit") ? (
                  <CheckCircle2 className="h-5 w-5 text-emerald-500" />
                ) : action.kind.includes("loan") ? (
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 mono text-xs text-slate-400">
                  {action.subscriber} • {formatDateTime(action.timestamp)}
                </p>
              </div>
            </div>
          ))}
