...
export default async function MobileHomePage() {
  ...
  const services = [
    ...
    },
    {
      ...
      status:
        subscriber.airtimeBalance <= 0
          ? "Need credit? Tap to bridge"
          : `${formatXaf(subscriber.airtimeBalance)} airtime`,
      icon: Wallet,
      href: "/mobile/wallet",
    },
    {
      title: "Travel Booster",
      status: "Not active",
      icon: Radio,
      href: "/mobile/wallet",
    },
    {
      ...
      status:
        subscriber.trustScore > 70
          ? "Eligible up to 25,000 XAF"
          : "Build trust score",
      icon: CheckCircle2,
      href: "/mobile/wallet",
    },
  ...
  return (
    <div className="space-y-5 md:grid md:grid-cols-2 md:gap-6 md:space-y-0">
        ...
        <div className="mt-4 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm opacity-90">
              Current speed: {latestEvent?.throughput?.toFixed(1) ?? "14.2"}{" "}
              Mbps
            </p>
            <p className="mono mt-1 text-xs opacity-80">
          ...
          <div className="flex items-end gap-1">
            {[1, 2, 3, 4, 5].map((bar) => (
              <span
                ...
                style={{
                  height: `${bar * 12}px`,
                  opacity: bar <= Math.round(score / 20) ? 1 : 0.35,
                }}
              />
            ))}
          </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1 md:col-span-2">
        {[
          ...
          <Link
            key={label}
            href={href}
            className="shrink-0 rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-800 shadow-card dark:bg-slate-900 dark:text-white"
          >
            {label}
          </Link>
        ))}
      ...
      <Card>
        ...
        <Badge tone={subscriber.trustScore > 70 ? "success" : "warning"}>
          {subscriber.trustScore > 70
            ? "SME loan eligible"
            : "Keep topping up regularly"}
        </Badge>
      </Card>
